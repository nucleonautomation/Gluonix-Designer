...
class Label:
    ...
    def __init__(self, Main, *args, **kwargs):
        ...
        if self._GUI is not None:
            self._Type = "Label"
            try:
                self._Config = ['Name', 'Auto_Dark', 'Background', 'Light_Background', 'Dark_Background', 'Foreground', 'Light_Foreground', 'Dark_Foreground', 'Border_Color', 'Light_Border_Color', 'Dark_Border_Color', 'Border_Size', 'Resize', 'Popup', 'Display', 'Left', 'Top', 'Width', 'Height', 'Animate_Left', 'Animate_Top', 'Animate_Width', 'Animate_Height', 'Animate_Time', 'Font_Size', 'Font_Weight', 'Font_Family', 'Align', 'Value', 'Hover_Background', 'Light_Hover_Background', 'Dark_Hover_Background', 'Hover_Foreground', 'Light_Hover_Foreground', 'Dark_Hover_Foreground', 'Hover_Border_Color', 'Light_Hover_Border_Color', 'Dark_Hover_Border_Color']
        ...
        try:
            ...
            if Dist == 0.0:
                def Snap_Same_Zero():
                    ...
                    if self._On_Animate:
                        self._On_Animate()
                self._GUI._Frame.after(0, Snap_Same_Zero)
                return
            ...
            def Worker():
                ...
                while not Stop.is_set():
                    ...
                    W = Start_Width + Dw * K
                    H = Start_Height + Dh * K
                    Cur = (float(X), float(Y), float(W), float(H))
                    if Cur != Last:
                        Last = Cur
                        def Post(C=Cur):
                            if not self._Widget.winfo_exists():
    ...
    def Create(self):
        try:
            ...
            self._Widget.config(background=self._Background, foreground=self._Foreground, font=self._Font, anchor=self._Align, text=self._Value, wraplength=int(round(Width)))
