...
class Stock_Button_Lite:
    def __init__(self, Global, Stock, Left, Top):
        try:
            self.Global = Global
            self.Stock = Stock
            self.Widget= []
            self.Type = 'Button_Lite'
            ...
            self.Label.Create()
            self.Label.Set(Path=self.Global['Image'](self.Type), Value=' Button')
        ...
        try:
            if ID:
                ...
                if len(Widget_Data)==1:
                    ...
                    setattr(Root, ID, self.Global['Gluonix'].Button_Lite(Root))
                    Widget_Data = self.Stock.Design.Database.Get(f"SELECT * FROM `Widget` WHERE `ID`='{ID}'", Keys=True)
                    Widget_Data = Widget_Data[0]
