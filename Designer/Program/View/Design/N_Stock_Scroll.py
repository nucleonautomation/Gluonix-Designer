...
class Stock_Scroll:
    def __init__(self, Global, Stock, Left, Top):
        try:
            ...
            self.Label.Create()
            self.Label.Set(Path=self.Global['Image'](self.Type), Value=' Scroll')
    ...
    def On_Click(self):
        try:
            ...
            if (Root_Type=='Frame' or Root_Type=='Canvas' or Root_Type=='Scroll') and Root_Level<100:
                Number = 1
                ...
                if Root_Level>99:
                    self.Global['Message'].Show('Error', 'Maximum Frame Level Reached')
                else:
        ...
        try:
            if ID:
                ...
                if len(Frame_Data)==1:
                    ...
                    setattr(Root, ID, self.Global['Gluonix'].Scroll(Root))
                    Frame_Data = self.Stock.Design.Database.Get(f"SELECT * FROM `Frame` WHERE `ID`='{ID}'", Keys=True)
                    Frame_Data = Frame_Data[0]
