...
class Stock_Canvas_Pie:
    def __init__(self, Global, Stock, Left, Top):
        try:
            self.Global = Global
            self.Stock = Stock
            self.Widget= []
            self.Type = 'Canvas_Pie'
            ...
            self.Label.Create()
            self.Label.Set(Path=self.Global['Image'](self.Type), Value=' Pie')
        ...
        try:
            if ID:
                ...
                if len(Widget_Data)==1:
                    ...
                    setattr(Root, ID, self.Global['Gluonix'].Canvas_Pie(Root))
                    Widget_Data = self.Stock.Design.Database.Get(f"SELECT * FROM `Item` WHERE `ID`='{ID}'", Keys=True)
                    Widget_Data = Widget_Data[0]
                    ...
                    Widget.Config(Start=Widget_Data['Start'], Extent=Widget_Data['Extent'])
