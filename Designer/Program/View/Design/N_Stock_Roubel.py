...
class Stock_Roubel:
    def __init__(self, Global, Stock, Left, Top):
        try:
            ...
            self.Label.Create()
            self.Label.Set(Path=self.Global['Image'](self.Type), Value=' Roubel')
        ...
        try:
            if ID:
                ...
                if len(Widget_Data)==1:
                    ...
                    setattr(Root, ID, self.Global['Gluonix'].Roubel(Root))
                    Widget_Data = self.Stock.Design.Database.Get(f"SELECT * FROM `Widget` WHERE `ID`='{ID}'", Keys=True)
                    Widget_Data = Widget_Data[0]
                    ...
                    Widget.Config(Foreground=Widget_Data['Foreground'], Border_Size=Widget_Data['Border_Size'], Border_Color=Widget_Data['Border_Color'], Radius=Widget_Data['Radius'], Display=True, Disable=bool(Widget_Data['Disabled']))
                    Widget.Config(Font_Size=Widget_Data['Font_Size'], Font_Weight=Widget_Data['Font_Weight'], Font_Family=Widget_Data['Font_Family'], Align=Widget_Data['Align'], Value=Widget_Data['Value'])
                    Widget.Config(Shadow_Size=Widget_Data['Shadow_Size'], Shadow_Color=Widget_Data['Shadow_Color'], Shadow_Full=Widget_Data['Shadow_Full'])
