...
class Configure_Canvas:
    def __init__(self, Global, Configure):
        try:
            ...
            #Shadow Label
            Fixture = self.Frame.Locate(15, 5, 45, 65)
            self.Shadow_Label = self.Global['Gluonix'].Label(self.Frame)
            self.Shadow_Label.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3])
            ...
            #Shadow Entry
            Fixture = self.Frame.Locate(20, 5, 60, 65)
            self.Shadow_Entry = self.Global['Gluonix'].Entry(self.Frame)
            self.Shadow_Entry.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3])
            ...
            #Shadow Color
            Fixture = self.Frame.Locate(7, 5, 85, 65)
            self.Shadow_Color = self.Global['Gluonix'].Label(self.Frame)
            self.Shadow_Color.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3])
            ...
            #Shadow Full Label
            Fixture = self.Frame.Locate(30, 5, 45, 72)
            self.Shadow_Full_Label = self.Global['Gluonix'].Label(self.Frame)
            self.Shadow_Full_Label.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3])
            ...
            #Shadow Full Check
            Fixture = self.Frame.Locate(7, 5, 85, 72)
            self.Shadow_Full_Check = self.Global['Gluonix'].Check(self.Frame)
            self.Shadow_Full_Check.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3])
            ...
            #Update Scroll
            self.Frame.Update(self.Move_Top_Check)
        ...
        try:
            if ID!=self.ID:
                ...
                if len(Frame_Data)==1:
                    ...
                    self.Resize_Width_Check.Set(bool(Frame_Data['Resize_Width']))
                    self.Shadow_Full_Check.Set(bool(Frame_Data['Shadow_Full']))
                    self.Resize_Height_Check.Set(bool(Frame_Data['Resize_Height']))
                    self.Move_Left_Check.Set(bool(Frame_Data['Move_Left']))
                    self.Move_Top_Check.Set(bool(Frame_Data['Move_Top']))
    ...
    def Update_Name(self):
        try:
            ...
            if self.Global['Custom'].Valid_Variable(Name) and len(TempName)==0:
                ...
                self.Configure.Design.Element.Tree.Edit(Name=Name)
                self.Name_Entry.Config(Border_Color='#000000', Border_Size=1)
            else:
    ...
    def Update_Shadow(self):
        try:
            ...
            if Shadow.isdigit():
                ...
                self.Shadow_Entry.Config(Border_Color='red', Border_Size=2)
        except Exception as E:
            self.Global['Error'](__class__.__name__+" -> "+inspect.currentframe().f_code.co_name+" -> "+str(E))
    ...
    def Update_Resize_Width(self):
        try:
            ...
            self.Configure.Design.Database.Post(f"UPDATE `Frame` SET `Resize_Width`='{int(Resize_Width)}' WHERE `ID`='{self.ID}'")
            if self.Element:
    ...
    def Update_Resize_Height(self):
        try:
            ...
            self.Configure.Design.Database.Post(f"UPDATE `Frame` SET `Resize_Height`='{int(Resize_Height)}' WHERE `ID`='{self.ID}'")
            if self.Element:
    ...
    def Update_Move_Left(self):
        try:
            ...
            self.Configure.Design.Database.Post(f"UPDATE `Frame` SET `Move_Left`='{int(Move_Left)}' WHERE `ID`='{self.ID}'")
            if self.Element:
    ...
    def Update_Move_Top(self):
        try:
            ...
            self.Configure.Design.Database.Post(f"UPDATE `Frame` SET `Move_Top`='{int(Move_Top)}' WHERE `ID`='{self.ID}'")
            if self.Element:
