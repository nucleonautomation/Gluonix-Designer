...
class Stock_Text:
    def __init__(self, Global, Stock, Left, Top):
        try:
            ...
            self.Label.Create()
            self.Label.Set(Path=self.Global['Image'](self.Type), Value=' Textarea')
        ...
        try:
            if ID:
                ...
                if len(Widget_Data)==1:
                    ...
                    setattr(Root, ID, self.Global['Gluonix'].Text(Root, Scroll=True))
                    Widget_Data = self.Stock.Design.Database.Get(f"SELECT * FROM `Widget` WHERE `ID`='{ID}'", Keys=True)
                    Widget_Data = Widget_Data[0]
                    ...
                    Widget.Tag('Tag1', Font_Size=12, Font_Weight='normal', Font='Times New Roman', Foreground='#000000', Background='#FFFFFF')
                    Widget.Tag('Tag2', Font_Size=20, Font_Weight='normal', Font='Times New Roman', Foreground='#FFFFFF', Background="red")
                    Widget.Add('Text Box\n', Tag='Tag1')
