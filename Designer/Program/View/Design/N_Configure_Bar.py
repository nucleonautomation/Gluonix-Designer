...
class Configure_Bar:
    def __init__(self, Global, Configure):
        try:
            ...
            Top += 7
            Fixture = self.Frame.Locate(25, 5, 3, Top)
            self.Progress_Label = self.Global['Gluonix'].Label(self.Frame)
            self.Progress_Label.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3], Resize=False, Move=False)
            self.Progress_Label.Config(Foreground='#000000', Value="Progress:", Font_Size=10, Font_Weight='normal', Align='w', Border_Size=0)
            self.Progress_Label.Create()
            
            #Progress Entry
            Fixture = self.Frame.Locate(60, 5, 28, Top)
            self.Progress_Entry = self.Global['Gluonix'].Entry(self.Frame)
            self.Progress_Entry.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3], Resize=False, Move=False)
            self.Progress_Entry.Config(Background='#FFFFFF', Foreground='#000000', Font_Size=9, Font_Weight='normal', Align='left', Border_Size=1)
            self.Progress_Entry.Bind(On_Key_Release=lambda E: self.Update_Progress())
            self.Progress_Entry.Create()
            
            #Zero Label
            Top += 7
            Fixture = self.Frame.Locate(25, 5, 3, Top)
            self.Zero_Label = self.Global['Gluonix'].Label(self.Frame)
            self.Zero_Label.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3], Resize=False, Move=False)
            self.Zero_Label.Config(Foreground='#000000', Value="Zero:", Font_Size=10, Font_Weight='normal', Align='w', Border_Size=0)
            self.Zero_Label.Create()
            
            #Zero Select
            Fixture = self.Frame.Locate(40, 5, 28, Top)
            self.Zero_Select = self.Global['Gluonix'].Select(self.Frame)
            self.Zero_Select.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3], Resize=False, Move=False)
            self.Zero_Select.Config(Background='#FFFFFF', Foreground='#000000', Font_Size=9, Font_Weight='normal', Align='center', Border_Size=1)
            self.Zero_Select.Add('Left')
            self.Zero_Select.Add('Top')
            self.Zero_Select.Add('Bottom')
            self.Zero_Select.Add('Right')
            self.Zero_Select.Bind(On_Change=lambda E: self.Update_Zero())
            self.Zero_Select.Create()
            
            #Update Scroll
            self.Frame.Update(self.Zero_Select)
        ...
        try:
            if ID!=self.ID:
                ...
                if len(Widget_Data)==1:
                    ...
                    self.Progress_Entry.Set(Widget_Data['Progress'])
                    self.Zero_Select.Set(Widget_Data['Zero'])
    ...
    def Update_Progress(self):
        try:
            ...
            if self.Global['Custom'].Is_Float(Progress):
                Progress = float(Progress)
                self.Progress_Entry.Config(Border_Color='#000000', Border_Size=1)
                self.Configure.Design.Database.Post(f"UPDATE `Widget` SET `Progress`='{Progress}' WHERE `ID`='{self.ID}'")
                self.Element.Config(Progress=Progress)
            else:
                self.Progress_Entry.Config(Border_Color='red', Border_Size=1)
        except Exception as E:
            self.Global['Error'](__class__.__name__+" -> "+inspect.currentframe().f_code.co_name+" -> "+str(E))
            
    def Update_Zero(self):
        try:
            Zero = self.Zero_Select.Get()
            self.Configure.Design.Database.Post(f"UPDATE `Widget` SET `Zero`='{Zero}' WHERE `ID`='{self.ID}'")
            self.Element.Config(Zero=Zero)
        except Exception as E:
            self.Global['Error'](__class__.__name__+" -> "+inspect.currentframe().f_code.co_name+" -> "+str(E))