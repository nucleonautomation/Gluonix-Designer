...
class Stock_Radio:
    def __init__(self, Global, Stock, Left, Top):
        try:
            ...
            self.Label.Create()
            self.Label.Set(Path=self.Global['Image'](self.Type), Value=' Radio')
    ...
    def On_Click(self):
        try:
            ...
            self.Stock.Design.Database.Post(f"INSERT INTO `Widget` (`ID`, `Name`, `Type`, `Root`, `Alignment`, `Value`, `Border_Size`, `Background`) VALUES ('{ID}', '{Name}', '{self.Type}', '{Root_ID}', '{self.Stock.Design.Alignment}', '{Name}', '0', 'False')")
            self.Create(ID, True)
        ...
        try:
            if ID:
                ...
                if len(Widget_Data)==1:
                    ...
                    setattr(Root, ID, self.Global['Gluonix'].Radio(Root))
                    Widget_Data = self.Stock.Design.Database.Get(f"SELECT * FROM `Widget` WHERE `ID`='{ID}'", Keys=True)
                    Widget_Data = Widget_Data[0]
                    ...
                    if Latest:
                        ...
                        Aspect_Ratio = 1.0
                        Min_W = int(Root_Width * 0.07)
                        Max_W = int(Root_Width * 0.15)
                        Min_H = int(Root_Height * 0.07)
                        Max_H = int(Root_Height * 0.15)
                        if Aspect_Ratio >= 1.0:
                    ...
                    Widget.Config(Url=bool(Widget_Data['Url']), Transparent=bool(Widget_Data['Transparent']), Rotate=Widget_Data['Rotate'], Compound=Widget_Data['Compound'])
                    Widget.Config(Secure=Widget_Data['Secure'])
