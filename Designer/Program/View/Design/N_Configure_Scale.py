...
class Configure_Scale:
    def __init__(self, Global, Configure):
        try:
            ...
            Top += 7
            Fixture = self.Frame.Locate(25, 5, 3, Top)
            self.Orient_Label = self.Global['Gluonix'].Label(self.Frame)
            self.Orient_Label.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3], Resize=False, Move=False)
            self.Orient_Label.Config(Foreground='#000000', Value="Orient:", Font_Size=10, Font_Weight='normal', Align='w', Border_Size=0)
            self.Orient_Label.Create()
            
            #Orient Select
            Fixture = self.Frame.Locate(40, 5, 28, Top)
            self.Orient_Select = self.Global['Gluonix'].Select(self.Frame)
            self.Orient_Select.Config(Width=Fixture[0], Height=Fixture[1], Left=Fixture[2], Top=Fixture[3], Resize=False, Move=False)
            self.Orient_Select.Config(Background='#FFFFFF', Foreground='#000000', Font_Size=9, Font_Weight='normal', Align='center', Border_Size=1)
            self.Orient_Select.Add('Horizontal')
            self.Orient_Select.Add('Vertical')
            self.Orient_Select.Bind(On_Change=lambda E: self.Update_Orient())
            self.Orient_Select.Create()
            
            #Update Scroll
            self.Frame.Update(self.Orient_Select)
        ...
        try:
            if ID!=self.ID:
                ...
                if len(Widget_Data)==1:
                    ...
                    self.Orient_Select.Set(Widget_Data['Orient'])
    ...
    def Update_Orient(self):
        try:
            Orient = self.Orient_Select.Get()
            self.Configure.Design.Database.Post(f"UPDATE `Widget` SET `Orient`='{Orient}' WHERE `ID`='{self.ID}'")
            self.Element.Config(Orient=Orient)
        except Exception as E:
            self.Global['Error'](__class__.__name__+" -> "+inspect.currentframe().f_code.co_name+" -> "+str(E))