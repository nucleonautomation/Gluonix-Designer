...
class Stock_Canvas_RectangleR:
    def __init__(self, Global, Stock, Left, Top):
        try:
            self.Global = Global
            self.Stock = Stock
            self.Widget= []
            self.Type = 'Canvas_RectangleR'
            ...
            self.Label.Create()
            self.Label.Set(Path=self.Global['Image'](self.Type), Value=' RectangleR')
    ...
    def On_Click(self):
        try:
            ...
            Level = Root_Level+1
            if (Root_Type!='Canvas' and Root_Type!='Scroll' and Root_Type!='Group' and Root_ID!='Root'):
                ...
                if len(Root_Container)>0:
                    ...
                    Root_Type = 'Frame'
            if  Root_Type=='Canvas' or Root_Type=='Scroll' or Root_Type=='Group':
                if Root_ID!='Root':
                    Widget = self.Stock.Design.Database.Get(f"SELECT * FROM `Frame` WHERE `ID`='{Root_ID}'", Keys=True)
                ...
                if Widget[0]['Type']=='Canvas' or Widget[0]['Type']=='Scroll':
                    ...
                    self.Stock.Design.Database.Post(f"INSERT INTO `Item` (`ID`, `Name`, `Type`, `Root`, `Left`, `Top`, `Value`, `Radius`) VALUES ('{ID}', '{Name}', '{self.Type}', '{Root_ID}', '40', '40', '{Name}', '0')")
                    self.Create(ID, True)
        ...
        try:
            if ID:
                ...
                if len(Widget_Data)==1:
                    ...
                    setattr(Root, ID, self.Global['Gluonix'].Canvas_RectangleR(Root))
                    Widget_Data = self.Stock.Design.Database.Get(f"SELECT * FROM `Item` WHERE `ID`='{ID}'", Keys=True)
                    Widget_Data = Widget_Data[0]
                    ...
                    if Latest:
                        ...
                        Half_W = Width // 2
                        Half_H = Height // 2
                        Left = random.randint(Half_W, max(Half_W, Root_Width - Half_W))
                        Top  = random.randint(Half_H, max(Half_H, Root_Height - Half_H))
                        Widget.Config(Width=Width, Height=Height, Left=Left, Top=Top, Radius=Widget_Data['Radius'])
                        self.Stock.Design.Database.Post(f"UPDATE `Item` SET `Width`='{Width}', `Height`='{Height}', `Left`='{Left}', `Top`='{Top}' WHERE `ID`='{ID}'")
                    else:
